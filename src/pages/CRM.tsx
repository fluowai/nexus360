    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="p-8 flex flex-col gap-5">
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest pl-1">Observações / Folow-ups</label>
            <textarea 
              className="modal-input min-h-[100px] resize-none"
              placeholder="Descreva o primeiro contato ou agendamento de reunião..."
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-end p-0 sm:p-4">
      ...
      <motion.div 
        ...
        className="bg-white w-full max-w-2xl h-full sm:h-[calc(100vh-32px)] sm:rounded-3xl shadow-2xl relative z-10 flex flex-col overflow-hidden"
      >
        {/* Header */}
        <div className="p-8 border-b border-gray-100 flex justify-between items-start">
          <div className="flex flex-col gap-2">
            <span className="text-[10px] font-bold text-primary bg-blue-50 px-2 py-1 rounded-md w-fit uppercase tracking-wider">
              {lead.status}
            </span>
          ...
          <button onClick={onClose} className="p-2 hover:bg-gray-100 rounded-full transition-colors font-bold text-gray-400">
             <X size={24} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-8 flex flex-col gap-8">
          ...
          <div className="grid grid-cols-2 gap-6 p-6 bg-gray-50 rounded-2xl">
            <div>
              ...
              <p className="text-sm font-semibold text-gray-700">{lead.source || 'Não informada'}</p>
            </div>
            <div>
              <p className="text-[10px] font-bold text-gray-400 uppercase mb-1">Valor do Contrato</p>
              <p className="text-sm font-bold text-green-600">R$ {lead.value?.toLocaleString()}</p>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="flex flex-col gap-6 pl-4 border-l-2 border-gray-100 relative">
                ...
                <div key={fu.id} className="relative">
                  ...
                  <div className="bg-white border border-gray-100 p-4 rounded-xl shadow-sm">
                    <div className="flex justify-between items-center mb-2">
                      <span className="text-xs font-bold text-primary uppercase">{fu.type}</span>
                      <span className="text-[10px] text-gray-400">{new Date(fu.createdAt).toLocaleDateString('pt-BR')}</span>
                    </div>
                    <p className="text-sm text-gray-700 leading-relaxed">{fu.content}</p>
                    {fu.scheduledAt && (
                      <div className="mt-3 flex items-center gap-2 text-xs font-semibold text-orange-600 bg-orange-50 px-2 py-1 rounded w-fit">
                        <Calendar size={12} />
                        Próximo Agendamento: {new Date(fu.scheduledAt).toLocaleDateString('pt-BR')}
                      </div>
                    )}
                  </div>
                </div>
              ...
              {lead.followUps?.length === 0 && (
                <p className="text-sm text-gray-400 italic">Nenhuma interação registrada ainda.</p>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="p-8 border-t border-gray-100 bg-gray-50/50">
          <h4 className="text-sm font-bold text-gray-700 mb-4">Registrar Nova Interação</h4>
          <form onSubmit={handleAddFollowUp} className="flex flex-col gap-4">
             ...
             <div className="flex gap-2">
               ...
               <button 
                disabled={submitting}
                className="bg-primary text-white p-4 rounded-xl hover:bg-blue-600 shadow-lg shadow-blue-200 transition-all self-end"
               >
                 <Send size={20} />
               </button>
             </div>
