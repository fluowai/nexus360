...
export function adminRoutes(prisma: PrismaClient) {
  ...
  router.post("/orgs", async (req: AuthRequest, res) => {
    console.log("[ADMIN] Requisição recebida em POST /api/admin/orgs:", req.body);
    if (req.user?.role !== 'SUPER_ADMIN') return res.status(403).json({ error: "Unauthorized" });
    ...
    try {
      ...
      console.error("[ADMIN] Erro ao criar agência:", error);
  ...
  // System Users (Other Super Admins)
  ...
  // Ad Accounts
  router.get("/ad-accounts", async (req: AuthRequest, res) => {
    if (req.user?.role !== 'SUPER_ADMIN') {
      return res.status(403).json({ error: "Access denied. Super Admin role required." });
    }
    const { orgId } = req.query;
